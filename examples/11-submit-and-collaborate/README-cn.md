...
> 这是 examples 系列里的第十一篇. 前置: [09-write-bullets](../09-write-bullets/README-cn.md) 和 [10-write-summary](../10-write-summary/README-cn.md) 已经跑完, 你那份 master 简历 ([students/john-doe/resume.md](../../students/john-doe/resume.md) 这种结构) 里已经有好几段经历的 bullet, 也已经有针对几个不同方向 (AI 工程师 / 数据分析师 / 后端工程师) 的 Summary. 这一节讲怎么把这份 master 简历真正变成投到公司招聘网站上的那一份 **PDF**, 以及怎么跟导师在 Google Doc 上协作改, 还有大多数学生最后栽跟头的地方: **排版微调**.
>
> **小术语备忘**: 本节及之后所有章节里出现的 "master 简历" 跟 09 / 10 章里说的 "master `resume.md`" 是同一个东西, 指 1 + N 简历法的母版 (`students/john-doe/resume.md` 这种), 永远不直接投递. 这门课早期版本里偶尔叫它 "大全简历", 后来统一成 "master 简历". 这两个词如果在仓库里还有残留, 都是指同一份文件.

## 1. 课程导读

跑完 09 和 10 之后, 你那份 master 简历的内容写完了, 但**还不能直接投**. 从一份 master 简历到能投出去的那份 PDF, 中间还差三件事:

- **派生**: master 简历里同时挂着多个方向的 Summary 和多段经历, 投一个具体岗位的时候要按这个岗位删减, 只留对得上的那份 Summary 和对得上的那几段经历, 拼出一份针对性版本
...
- **每个岗位单独一份 PDF**. 这是 05 教的"一份 master + 多份针对性派生"的最终落地点: master 简历只有一份, 但投到不同公司, 不同岗位方向的派生 PDF 可以有好多份
- **应届生 / 学生一律 1 页**, 多 1 行都不行; 1 到 3 年工作经验也优先 1 页, 经历实在多到值得展开偶尔可以 2 页; 3 年以上 2 页 OK, 但 3 页就过分了

PDF 是终点, 那 PDF 是怎么来的? Google Doc 导出. Google Doc 里的内容又是怎么来的? 从 master 简历复制过来再按目标岗位删减. 所以整条链路是:

**GitHub 上的 master 简历 (内容源头) → Google Doc (排版 + 跟导师协作) → 派生 PDF (投到公司)**
...
**学生的关键动作: 把 Google Doc 上的改动同步回 GitHub**. 导师在 Google Doc 上改了字 (这是好事, 也是你想要的), **你有责任把这些改动复制粘贴回 GitHub 上的 master 简历 `resume.md`**, 让 GitHub 上的内容跟得上 Google Doc 上的最新版. 为什么要回写? 因为下次你再让 AI 帮你改简历的时候, AI 读的是 GitHub 上的版本. 如果 GitHub 落后了, AI 看到的就是旧版, 改出来的东西也是基于旧版, 等于把导师上一轮的改进抹掉了. 回写的成本很低, 复制粘贴几句话而已; 但漏了回写, 后面就会出现"Google Doc 版本和 GitHub 版本不一致, AI 帮我改还基于旧版"这种麻烦.

一句话记住:

> **导师在 Google Doc 上改了什么, 你就把那部分文字回写到 GitHub 上的 master 简历里.**

### 3.3 PDF 的角色: 定格输出, 谁看都一样
...
```mermaid
flowchart TD
    GH["GitHub 上的 master 简历 + 所有素材<br/>(AI 工作的弹药库)"]
...
Google Doc 有个 **Tab 功能**, 一个 doc 的左侧栏可以建多个 tab, 每个 tab 是独立的一份文档. 这个功能正好对应我们"一份 master + 多份针对性派生"的玩法.

一份 Google Doc 里同时挂 **1 份 master tab + N 份派生 tab**:

- **master tab (1 个)**: 跟 GitHub 上的 master 简历 [resume.md](../../students/john-doe/resume.md) 内容一一对应, 把所有方向的 Summary, 所有经历的 bullet, 所有技能都堆在一起, 长度可能超过 1 页 (因为是全的). 这个 tab 是工作 tab, 不会用来投
- **派生 tab (N 个)**: 每个对应一个具体岗位方向, 比如"AI 工程师""数据分析师""Cascadia AI Solutions Engineer""NovaRisk Data Analyst". 每个派生 tab 都是从 master tab 复制一份过来, 按目标 JD 删减后剩下的那 1 页内容, 这才是真的拿去投的版本

一个派生 tab 怎么生成 (操作步骤):

1. 在 Google Doc 里点开 master tab, 全选, 复制
...
这一节的核心规矩, §3.2 末尾已经说过, 这里再展开讲讲为什么这个规矩这么重要, 还有怎么具体做.

### 5.1 反例: 在 Google Doc 上改了字, 没回写 GitHub

最常见的崩盘场景:

- 周一: 你跑完 09 和 10, GitHub 上的 master 简历 `resume.md` 写完了, 复制粘贴到 Google Doc master tab. 导师拿到链接看
- 周三: 导师在 Google Doc master tab 上直接划改了几条 bullet 的用词, 觉得读起来更顺. 同时也在派生 tab 上微调了 Summary 的用词
- 周三晚上: 你没回 GitHub, 觉得"反正改完了, 就这样吧"
- 两周后: 你做了个新项目, 想让 AI 帮你把这段新经历也写成 bullet 加进 master 简历. AI 打开 GitHub 上的 master 简历看, 看到的是**周一那份没改过的旧版本**
...
> **简历的内容只有一处源头, 就是 GitHub 上的 master 简历. Google Doc 上的改动必须最终回流到这里, 不然下一轮 AI 帮你改的时候, 会基于落后的源头.**
...
**Step 1: 确认 master 简历完工**

John 跑完 09 和 10, master 简历 [resume.md](../../students/john-doe/resume.md) 已经写满: §1 挂了 3 份不同方向的 Summary (AI 工程师 / 数据分析师 / 后端工程师); §3 是全套技能表; §4 是 6 段经历的 bullet (其中 Cedar Ridge 这段经历分别从 AI 角度和数据分析角度写了两组 bullet).

**Step 2: 决定投哪家, 用哪个方向**

John 看到 Cascadia 在招 AI Solutions Engineer. 他从 master 简历里挑出 AI 工程师方向的那一份 Summary, 和能撑得起这个方向的几段经历的 bullet 当素材.
...
> **阅读建议**: 下面 7 条铁律建议第一次完整读一遍, 之后正式排版时回查就行. 其中 **7.2 一致性**是最核心的一条, 其它 6 条本质上都是它的具体落地. 如果时间紧只看一条, 看 7.2.
...
### 9.1 GitHub 上的 master 简历里全部放

master 简历是你的资料库, 后面派生要用, 所以**全部放**:
...
排版微调反馈环是这一节最重要的能力. **这一步躲不掉, 也不该躲**. 而且这个手动过程其实在帮你建立对**排版**的直觉, 以后看一份简历能 3 秒判断"这份排版做没做好".
...
| 时间 | 你做 | 导师做 |
|---|---|---|
| 第 1 天 | GitHub 上的 master 简历写完 (跑完 09 和 10), 复制到 Google Doc master tab | 等 |
| 第 2 天 | 把 Google Doc 链接发给导师, 招呼一声"请帮我看 master tab" | 收到通知 |
| 第 2 到 4 天 | 等 | 在 Google Doc master tab 上直接划改字, 加评论 (主要看内容: bullet 措辞, Summary 定位, 技能覆盖) |
...
| 第 10 天 | 导出 PDF, 投递 | 无 (导师在第 8-9 天已终审定稿) |

后续每加一个目标 JD, 在 Google Doc 上新建 1 个派生 tab, 5 天迭代 1 轮就够 (master 没有大改的话).
...
- 05 教"一份 master + 多份派生"的简历法
- 06 教项目素材三条路
- 07 / 08 跑完 6 阶段链路, 拿到 case 文档
- 09 把 case 压成 bullet, 直接写进 master 简历 §4 Experience
- 10 从已经写好的 bullet 反推不同方向的 Summary, 直接写进 master 简历 §1 Summary
- 11 (本节) **从 master 派生针对一个 JD 的 markdown 和 Google Doc 派生 tab, 调排版, 导 PDF, 投到公司**

到这里你已经能完整跑通 **"设计 case → 写 case → 写 bullet → 写 Summary → 派生针对性版本 → 投递 PDF"** 整条链路了.

下一节 [12-maintain-new-projects](../12-maintain-new-projects/README-cn.md) 讲: 你投出去 PDF 之后, 几个月后又做了**新的项目** (新实习, 新 side project), 怎么把新经历**接入**到现有 master 里, 多个 `qualify-for-<JD>` 子目录怎么共存. 这是简历进入"持续维护期"的第一节.
