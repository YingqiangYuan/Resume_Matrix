...
flowchart TD
    ...
    BULLET["⑤  写成 bullet 加进大全<br/>(bullet-writer / -reviewer)"]
    SUMMARY["⑥  Summary 同步更新<br/>(summary-writer / -reviewer)"]
    DERIVE["⑦  派生针对性简历<br/>(11 的 GitHub + Google Doc 双轨)"]
    SUBMIT["⑧  投递 PDF"]
    FEEDBACK["⑨  收反馈<br/>(招聘官 / 面试官 / mock / mentor)"]
    ...
    DERIVE --> SUBMIT
    SUBMIT --> FEEDBACK
    FEEDBACK -.->|"反馈告诉你下一个项目<br/>该补什么、该突出什么"| DESIGN
...
flowchart TD
    ...
    BUILD --> ASSET
    ASSET --> LOOP
    LOOP -.->|"闭环回归每一节,<br/>把课件变成日常动作"| BUILD
