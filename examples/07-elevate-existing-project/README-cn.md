...
#### 当目标是一整个 Job Family, 而不是一家具体公司时

上面默认的场景是"John 已经锁定 Cascadia 这一家公司的这一个 JD". 但很多时候你面对的不是这么单一: 你想投的是整条 Job Family (比如 AI/ML Engineer), 手上只是先拿到了其中一家公司的 JD 当起点, 后面这个方向要投的是很多家公司.

这时候如果直接把这一家 JD 的 landscape 当成阶段 2-3 的锚点, 有个风险: 这家公司的 JD 有它自己的偏好和措辞 (可能特别强调某个工具, 某种考核方式), 照着它诊断出的 gap 和设计出的 case, 很可能只对这一家公司管用, 换一家同 family 的公司投递就不完全对得上. 这也是本节最开始那位学生的困惑: 一个具体 JD 改出来的东西不一定有 family 层面的普适性.

解法不是"随便挑一份看起来更有代表性的 JD", 而是在跑阶段 1 之前多做一步:

1. 在这条 Job Family 里, 再挑 3 到 5 份有代表性的 JD (不同公司, 同一个方向).
2. 对每份 (或至少几份) 都跑一遍 `understand-landscape`, 手上会有 3 到 5 份 landscape, 而不是 1 份.
3. **人工做一次取舍决策**: 通读这几份 landscape 和 JD 原文, 看这几家公司在这个 family 里的侧重点是不是完全一致. 如果侧重点有分歧 (比如一部分公司的卖点是"后端 + 云计算", 另一部分是"AI Agent 落地到业务逻辑"), 这是一个战略选择, AI 替你做不了. 你要自己想清楚打哪种牌: 主打 A, 反过来主打 B, 还是两个都要 (代价是两头都要花更多力气做深).
4. 把这个决策告诉 AI, 让它把这 3 到 5 份 landscape **综合**成一份新的 landscape/JD 画像, 明确写清楚"这份综合画像代表的是哪种打法, 舍弃了哪些不在这个打法里的分支要求". 这份综合产物替代原来单一的 `job-description.md`, 继续喂给阶段 2 `qualify-gap-analyze` 和阶段 3 `mini-project-design`, 链路完全不用改.

> 注: 如果这几份 JD 根本是**八竿子打不着的不同方向** (比如一份 AI Engineer 一份纯前端), 说明它们压根不在同一个 family 里, 不能合并, 只能先选一个当主线, 单独跑一遍这套 6 阶段链路.

这一步不是每次都要做. 如果你这次就是奔着一家具体公司去的 (像 John 投 Cascadia), 单 JD 走 07 默认流程就够了. 只有当你的目标本来就是"这整条 family 我都要投", 才值得先花这一轮做综合. 而且这一步做在阶段 1, 好处是下游全部继承: 阶段 2 的 gap 诊断, 阶段 3 的 case 设计, 一路到 [09-write-bullets](../09-write-bullets/README-cn.md) 压出来的 bullet, 全都是针对这个 family 综合画像写的, 不用等到写 bullet 那一步才回头补"通用性".
