...
> **小术语备忘 (角度 vs 方向)**: 09 里 Bullet Set 用"角度" (angle, 一段经历的多个写法侧重, 例如 Cedar Ridge 这一段同时有 "AI emphasis" 角度和 "Data Analytics emphasis" 角度), 10 之后 Summary Variant 用"方向" (direction, 整个人定位往哪个 Job Family 走, 例如 "AI Engineer 方向""Data Analyst 方向"). 这两个词颗粒度不同: 一份 master 简历同时挂 **N 个方向** (Summary Variant) 和 **M 个角度** (Bullet Set), 派生时按 1 个方向选 1 个 Summary Variant + 那个方向覆盖的几个 Bullet Set. 听起来绕, 看完 §3 实例就懂了.
...
3. skill 起草初版 Summary (2 行, 200 到 300 字符), 显式标出 3 个组成部分 (Identity / Strength / Evidence). 长得像这样:
...
## 5. summary-reviewer 用法详解 (可选, 微调用)
...
下一节 [11-submit-and-collaborate](../11-submit-and-collaborate/README-cn.md) 教怎么把这份 master 真正派生成投出去的 PDF, 以及怎么跟导师在 Google Doc 上协作. 那是简历从"在 GitHub 上写得很漂亮"到"招聘官在公司招聘网站上看到"中间的最后一公里.
