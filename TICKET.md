...
- [ ] Work through every file on the run-list: read the 13 lessons under `examples/01-hiring/` through `examples/13-final-synthesis/` in order, then skim `students/john-doe/` to see what each skill's output looks like
- [ ] Come back to `/learn-this-project-absorb` in **Context-dive mode** whenever a specific spot needs unpacking (give it a `file:line` or paste the code snippet)
- [ ] Be able to explain **why the 1+N method derives targeted resumes by deletion only, never addition** — i.e., why subtraction scales and what internal-consistency property addition would break (lesson `examples/05-resume-matrix/`)
